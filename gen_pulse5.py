...
# Split by <div class="case-study-container">
parts = html.split('<div class="case-study-container">')
top_part = parts[0]

# Split the second part by the first <script> tag (or just the closing </body>)
# Let's find the closing </div> of case-study-container. 
# It's safer to just split by <script>
bottom_parts = parts[1].split('<script>')
bottom_part = '<script>' + '<script>'.join(bottom_parts[1:])
